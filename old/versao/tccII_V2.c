   ...
   for (i=2;i<=(M-1);i++) {   // 2o. for, dire�o Z 
    	...
    	for (j=2;j<=(L-1);j++) {        // 3o. for, dire�o r
         ...
         if(T[L*(i-1)+(j-1)]>=1) {  // Caso sat
   			...
   			TN[L*(i-1)+(j-1)]=(AE*PE+AW*PW+AP*PP+AN*PN+AS*PS)+T[L*(i-1)+(j-1)];

              
      		} // fim do 3o. for, j

       T[L*(i-1)+(L-1)]=TN[L*(i-1)+(L-2)]; //  CONDI�O DE FRONTEIRA EM r=R, isolamento

	} //  fim do 2o. for, i
   
   for (i1=2;i1<=(M-1);i1++) {    // envelhecimento da TN
         for (j=2;j<=(L-1);j++) {
         	T[M*(i1-1)+(j-1)]=TN[M*(i1-1)+(j-1)];
          }
       }  
          
   for (j=2;j<=(L-1);j++){
		T[L*(M-1)+(j-1)]=TN[L*(M-2)+(j-1)]; //  maquiagem em Z=H
    }
   
      T[L*(M-1)+(L-1)]=TN[L*(M-2)+(L-2)];
      T[L*(M-1)]=T[L*(M-2)];
