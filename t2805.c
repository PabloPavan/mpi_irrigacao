...
const int L=256;   // no. de pontos em r, varia com i
const int M=256;   // no. de pontos em z, varia com j
...
const double a=0.467; //%0.674;%0.526;%;  % coeficiente do Psi na equação do potencial */
const double m=0.934; //%0.768;%0.519;%;  % expoente na equação do potencial */
const double n=2/(1-m); //expoente na equação do potencial, Burdines 
const double Ts=0.76; //teor de umidade de saturação, dimensional
const double tempo=nn*dt;
const double Ko=0.000004; //condutividade hidráulica de saturação
...
const double qo = 0.035; //fluxo de transpiração mínimo (m3/hora)
const double qM = 0.09; //fluxo de transpiração máximo
   ...
   for (int j0=1; j0<M; j0++){
    ...
    for (ii=1; ii < nc; ii++){ 	// contador dos chutes do PI
        ...
        for (int j1=1; j1<M; j1++){
          	for (int i=1; i<L; i++){
                 T[i][j1]=T00;  //condição inicial para toda a malha 
            }        
       	}
