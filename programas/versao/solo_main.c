...
double *T,*TN,*TT1;
 ...
 int L=6,    // no. de pontos em r, varia com j
...
double  Tr=0.07,  // teor de umidade residual, dimensional  
       ...
       T00=0, // teor de umidade inicial adimensi
       T1=0,
       Tf=0,
       te=0;
...
if((Tatual.maq=(int *) malloc(maq*sizeof(int)))==NULL){
   puts("Nao existe memoria suficiente para este sistema.");
   exit(1);
	}
if((Tatual.inio=(int *) malloc(maq*sizeof(int)))==NULL){
   puts("Nao existe memoria suficiente para este sistema.");
   exit(1);
	}
if((Tatual.fim=(int *) malloc(maq*sizeof(int)))==NULL){
   puts("Nao existe memoria suficiente para este sistema.");
   exit(1);
	}
}
...
int numlins, extra, linnum=0, k=2;  // distribui linhas aos clientes
	...
	if (k == 2) {  // I inicial
       ...
       Tatual.maq[i] = tids[i];         // Váriavel para divisao da matriz T
       Tatual.inio[i] = Li;
       Tatual.fim[i] = Lf; 
    }
...
for(oti=0; oti < nworkers; oti++) {

printf ("Teste %d %d %d\n", Tatual.maq[oti], Tatual.inio[oti],Tatual.fim[oti]);

   if (Tatual.inio[oti] == 2) {
        ctr = Tatual.fim[oti]*L;
 	pvm_initsend(PvmDataDefault);
        pvm_pkint  (&oti,1 ,1 );
	pvm_pkint  (&ctr,1 ,1 );
 	pvm_pkdouble (&T[0], ctr ,1);
 	pvm_send (Tatual.maq[oti],3);			// Envio=3

                 }
     else {
	pos = (L*(Tatual.inio[oti]-3));
 	ctr = (Tatual.fim[oti]-(Tatual.inio[oti]-3))*L;
       	pvm_initsend(PvmDataDefault);
        pvm_pkint  (&pos,1 ,1);
	pvm_pkint  (&ctr,1 ,1);
 	pvm_pkdouble (&T[pos], ctr ,1);
 	pvm_send (Tatual.maq[oti],3);
...
main (int argc, char *argv[] ) {
   ...
   aloca (atoi(argv[2])); //Aloca memoria para o sistema
